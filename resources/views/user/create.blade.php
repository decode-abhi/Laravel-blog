...
<head>
    ...
    <style>
        ...
        input,select, textarea { padding: 12px; font-size: 16px; border: 1px solid #ccc; border-radius: 6px; resize: vertical; } input[type="text"]:focus, textarea:focus { border-color: #007bff; outline: none; } 
...
<body>
    ...
    <form action="{{route('user.store')}}" method="post" enctype="multipart/form-data">
        ...
        <input type="email" name="email" value="{{old('email')}}"  id="" placeholder="enter email">
        <select name="role" id="">
            <option value="0">Select Role</option>
            <option value="1">Admin</option>
            <option value="0">Customer</option>
        </select>
        <input type="password" name="password" value=""  id="" placeholder="enter passowrd">
        <input type="file" name="profile_image" id="" value="{{old('image')}}">
